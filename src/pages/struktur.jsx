    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const strukturResponse = await axios.get('http://127.0.0.1:8000/api/struktur');
                setStrukturDesa(strukturResponse.data);

                // Fetch data for Perangkat Desa
                const perangkatResponse = await axios.get('http://127.0.0.1:8000/api/perangkat-desa');
                    ...
                    strukturDesa.map((struktur) => (
                        <Grid item size={{ xs: 12, sm: 6, md: 4, lg: 3 }} key={struktur.id}>
                            <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column', boxShadow: 3 }}>
                                <CardMedia
                                    component="img"
                                    image={`http://127.0.0.1:8000${struktur.gambar}`}
                                    alt={struktur.nama}
                                    sx={{ 
                                        height: 250, 
                                        objectFit: 'cover'
                    ...
                    perangkatDesa.map((perangkat) => (
                        <Grid item size={{ xs: 12, sm: 6, md: 4, lg: 3 }} key={perangkat.id}>
                            <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column', boxShadow: 3 }}>
                                <CardMedia
                                    component="img"
                                    image={`http://127.0.0.1:8000${perangkat.foto}`}
                                    alt={perangkat.nama}
                                    sx={{ 
                                        height: 250, 
                                        objectFit: 'cover'
